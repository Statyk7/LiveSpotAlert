...
class ProcessImageForLiveActivityUseCase implements UseCase<String?, ProcessImageForLiveActivityParams> {
  @override
  Future<Either<Failure, String?>> call(ProcessImageForLiveActivityParams params) async {
    try {
      ...
      debugPrint("Original image size: ${originalSizeKB.toStringAsFixed(1)} KB");
      ...
      if (image.width > params.maxDimension || image.height > params.maxDimension) {
        ...
        double scale = params.maxDimension / (image.width > image.height ? image.width : image.height);
      ...
      Uint8List processedImageBytes = Uint8List.fromList(img.encodeJpg(image, quality: params.initialQuality));
      
      // Check if still too large, reduce quality further if needed
      int quality = params.initialQuality;
      while (processedImageBytes.length > (params.maxEncodedSizeKB * 1024 * 0.75) && quality > params.minQuality) {
        quality -= 15;
        processedImageBytes = Uint8List.fromList(img.encodeJpg(image, quality: quality));
        debugPrint("Reduced quality to $quality%, size: ${(processedImageBytes.length / 1024).toStringAsFixed(1)} KB");
      }
      ...
      debugPrint("Final optimized image: ${finalSizeKB.toStringAsFixed(1)} KB base64 (${(processedImageBytes.length / 1024).toStringAsFixed(1)} KB raw)");
      
      // Final size check
      if (finalSizeKB > params.maxEncodedSizeKB) {
        debugPrint("Warning: Image still large at ${finalSizeKB.toStringAsFixed(1)} KB");
      }
      ...
      return Left(ImageProcessingFailure(message: 'Failed to process image: $e'));
    }
  }
}

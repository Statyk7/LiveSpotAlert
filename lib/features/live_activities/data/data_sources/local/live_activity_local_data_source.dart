...
class LiveActivityLocalDataSourceImpl implements LiveActivityLocalDataSource {
  ...
  static const String _activeConfigKey = '${AppConstants.liveActivitiesKey}_active_config';
  ...
  Future<List<ActivityDto>> getLiveActivities() async {
    try {
      ...
      AppLogger.debug('Retrieved ${activities.length} live activities from local storage');
      return activities;
    } catch (e, stackTrace) {
      AppLogger.error('Error retrieving live activities from local storage', e, stackTrace);
      return [];
    }
  }
  ...
  Future<ActivityDto?> getLiveActivityById(String id) async {
    try {
      ...
      AppLogger.error('Error retrieving live activity by id: $id', e, stackTrace);
      return null;
    }
  }
  ...
  Future<void> saveLiveActivity(ActivityDto activity) async {
    try {
      ...
      AppLogger.error('Error saving live activity: ${activity.title}', e, stackTrace);
      rethrow;
    }
  }
  ...
  Future<void> saveLiveActivities(List<ActivityDto> activities) async {
    try {
      ...
      AppLogger.debug('Saved ${activities.length} live activities to local storage');
    } catch (e, stackTrace) {
      AppLogger.error('Error saving live activities to local storage', e, stackTrace);
      rethrow;
    }
  }
  ...
  Future<List<ActivityDto>> getLiveActivityHistory({
    ...
    try {
      ...
      // Apply filters
      if (geofenceId != null) {
        activities = activities.where((a) => a.geofenceId == geofenceId).toList();
      }
      
      if (startDate != null) {
        activities = activities.where((a) => DateTime.parse(a.createdAt).isAfter(startDate)).toList();
      }
      
      if (endDate != null) {
        activities = activities.where((a) => DateTime.parse(a.createdAt).isBefore(endDate)).toList();
      }
  ...
  Future<ActivityDto?> getActiveConfiguration() async {
    try {
      ...
      AppLogger.error('Error retrieving active live activity configuration', e, stackTrace);
      return null;
    }
  }
  ...
  Future<void> saveActiveConfiguration(ActivityDto configuration) async {
    try {
      ...
      AppLogger.debug('Saved active live activity configuration: ${configuration.title}');
    } catch (e, stackTrace) {
      AppLogger.error('Error saving active live activity configuration', e, stackTrace);
      rethrow;
    }
  }
  ...
  Future<void> clearActiveConfiguration() async {
    try {
      ...
      AppLogger.error('Error clearing active live activity configuration', e, stackTrace);
      rethrow;
    }
  }
}
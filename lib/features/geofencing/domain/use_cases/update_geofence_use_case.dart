...
class UpdateGeofenceUseCase implements UseCase<Geofence, UpdateGeofenceParams> {
  ...
  Future<Either<Failure, Geofence>> call(UpdateGeofenceParams params) async {
    ...
    final existingGeofenceResult = await _geofencingService.getGeofenceById(params.geofence.id);
  ...
  Failure? _validateParams(UpdateGeofenceParams params) {
    ...
    if (geofence.radius > 10000) { // 10km max
      return const LocationFailure(message: 'Radius cannot exceed 10,000 meters');
    }
    
    if (geofence.radius < 10) { // 10m minimum for reliability

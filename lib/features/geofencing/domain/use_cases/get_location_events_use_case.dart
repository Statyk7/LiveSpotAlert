...
class GetLocationEventsUseCase
    implements UseCase<List<LocationEvent>, GetLocationEventsParams> {
  const GetLocationEventsUseCase(this._geofencingService);

  final GeofencingService _geofencingService;

  @override
  Future<Either<Failure, List<LocationEvent>>> call(
      GetLocationEventsParams params) async {
    // Validate date range if provided
    if (params.startDate != null && params.endDate != null) {
      if (params.startDate!.isAfter(params.endDate!)) {
        return Left(
            LocationFailure(message: 'Start date cannot be after end date'));
      }
    }
...
class GetLocationEventsParams extends Equatable {
  ...
  const GetLocationEventsParams.forDateRange(
      DateTime startDate, DateTime endDate,
      {int? limit})
      : this(startDate: startDate, endDate: endDate, limit: limit);

  const GetLocationEventsParams.recent({int limit = 20}) : this(limit: limit);

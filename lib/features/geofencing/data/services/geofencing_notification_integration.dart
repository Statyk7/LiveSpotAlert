...
class GeofencingNotificationIntegration {
  ...
  Future<Either<Failure, void>> handleGeofenceEntry(
    ...
    try {
      AppLogger.info(
          'Handling geofence entry notification for: ${geofence.name}');
      ...
      final config = configResult
          .getOrElse(() => throw Exception('Failed to load config'));

      // Check if notifications are enabled
      if (!config.isEnabled) {
        AppLogger.info(
            'Notifications disabled, skipping notification for ${geofence.name}');
      ...
      return await result.fold(
        ...
        (_) async {
          ...
          AppLogger.info(
              'Geofence entry notification shown for: ${geofence.name}');
          return const Right(null);
        },
      );
    } catch (e, stackTrace) {
      AppLogger.error(
          'Error handling geofence entry notification', e, stackTrace);
      return Left(NotificationFailure(
          message: 'Failed to handle geofence entry notification: $e'));
    }
  }
  ...
  Future<Either<Failure, void>> handleGeofenceExit(
    ...
    try {
      AppLogger.info(
          'Handling geofence exit notification for: ${geofence.name}');

      // Dismiss the active notification for this geofence
      if (_activeNotifications.contains(geofence.id)) {
        final dismissResult =
            await notificationsService.dismissGeofenceNotification(geofence.id);

        await dismissResult.fold(
          (failure) async {
            AppLogger.warning(
                'Failed to dismiss geofence notification: ${failure.message}');
          },
          (_) async {
            _activeNotifications.remove(geofence.id);
            AppLogger.info(
                'Geofence notification dismissed for: ${geofence.name}');
          },
        );
      }
      ...
      if (config.isEnabled && geofence.isActive) {
        ...
        exitResult.fold(
          (failure) => AppLogger.warning(
              'Failed to show exit notification: ${failure.message}'),
          (_) {
            AppLogger.info(
                'Geofence exit notification shown for: ${geofence.name}');

            // Auto-dismiss exit notification after 5 seconds
            Timer(const Duration(seconds: 5), () async {
              await notificationsService
                  .dismissGeofenceNotification('${geofence.id}_exit');
            });
          },
        );
      }
      ...
      AppLogger.error(
          'Error handling geofence exit notification', e, stackTrace);
      return Left(NotificationFailure(
          message: 'Failed to handle geofence exit notification: $e'));
    }
  }
  ...
  Future<Either<Failure, void>> handleGeofenceDwell(
    ...
    try {
      AppLogger.info(
          'Handling geofence dwell notification for: ${geofence.name}');
      ...
      if (!_activeNotifications.contains(geofence.id)) {
        ...
        result.fold(
          (failure) => AppLogger.warning(
              'Failed to show dwell notification: ${failure.message}'),
          (_) {
            _activeNotifications.add(geofence.id);
            AppLogger.info(
                'Geofence dwell notification shown for: ${geofence.name}');
          },
        );
      }
      ...
      AppLogger.error(
          'Error handling geofence dwell notification', e, stackTrace);
      return Left(NotificationFailure(
          message: 'Failed to handle geofence dwell notification: $e'));
    }
  }

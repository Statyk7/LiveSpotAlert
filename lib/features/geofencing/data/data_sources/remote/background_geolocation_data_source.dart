...
class BackgroundGeolocationDataSourceImpl
    implements BackgroundGeolocationDataSource {
  BackgroundGeolocationDataSourceImpl() {
    _initializeStreams();
  }

  final StreamController<LocationEventDto> _geofenceEventController =
      StreamController<LocationEventDto>.broadcast();
  ...
  void _initializeStreams() {
    ...
    bg.BackgroundGeolocation.onGeofence((bg.GeofenceEvent event) {
      try {
        AppLogger.info(
            'Geofence event received: ${event.action} for ${event.identifier}');

        final locationEventDto =
            LocationEventDto.fromBackgroundGeolocationEvent({
    ...
    bg.BackgroundGeolocation.onProviderChange((bg.ProviderChangeEvent event) {
      AppLogger.debug(
          'Provider changed: GPS enabled: ${event.gps}, Network enabled: ${event.network}');
    });
  }
    ...
    try {
      await bg.BackgroundGeolocation.ready(bg.Config(
        ...
        backgroundPermissionRationale: bg.PermissionRationale(
          ...
          message:
              "This app collects location data to enable geofence monitoring even when the app is closed or not in use.",
  ...
  Future<bool> isRunning() async {
    try {
      ...
      AppLogger.error(
          'Error checking if BackgroundGeolocation is running', e, stackTrace);
      return false;
    }
  }
